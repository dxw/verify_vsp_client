...
RSpec.describe VerifyVspClient::ServiceProvider do
  describe "#generate_request" do
    it "makes a POST request to the VSP for an authenticate request and returns the response as a Hash" do
      ...
      response = VerifyVspClient::ServiceProvider.new.generate_request
    ...
    it "raises VerifyVspClient::ResponseError if an error response is received" do
      request_stub = stub_vsp_generate_request(stubbed_auth_request_error_response)

      expect {
        VerifyVspClient::ServiceProvider.new.generate_request
      }.to raise_error(VerifyVspClient::ResponseError)
  ...
  describe "#translate_response" do
    let(:saml_response) { VerifyVspClient::FakeSso::IDENTITY_VERIFIED_SAML_RESPONSE }
    ...
    it "makes a POST request to the VSP to translate the SAML response and returns the result as a Hash" do
      ...
      response = VerifyVspClient::ServiceProvider.new.translate_response(saml_response, request_id, level_of_assurance)
    ...
    it "raises VerifyVspClient::ResponseError if an error response is received" do
      ...
      expect {
        VerifyVspClient::ServiceProvider.new.translate_response(saml_response, request_id, level_of_assurance)
      }.to raise_error(VerifyVspClient::ResponseError)
